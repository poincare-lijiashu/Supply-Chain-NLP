...
- **相邻混填 12%**："奶粉和合同一起寄"按第一项定类目（词序最小对比对；第二件 70% 用中性背景物品池，防类目代表词被跨类标签污染）
- **馈赠句式**："给小孩寄的奶粉"、"给客户的合同"——真实寄件高频句式，收件人亲疏不影响类目
...
| 快速模型调参 | fastText autotune 300s | 自动搜索 wordNgrams/epoch/lr | **96.43% / 96.40%** | 可追平，但需逐场景调参 |
| 主模型 | BERT 微调 | bert-base-chinese + 全连接层，AdamW lr=5e-5，batch=128，4 epochs | **96.50%**，Macro F1 0.9673 | 零调参泛化 + 长程组合能力，397MB |
| 压缩① | 动态量化 int8 | quantize_dynamic({Linear}) | **96.52%**，390→146MB（**2.68×**） | 精度无损 |
| 压缩② | 知识蒸馏 | 教师 BERT → 学生 BiLSTM(128/256/2层)，KL·T²(T=2.0) + CE，α=0.7，6 epochs | **96.50%（与教师打平）**，20.9MB（约 **1/19**），CPU 推理 **1.3ms**（约 **15×**） | **最终上线口径** |
| 可选 | L1 剪枝 30% | encoder.query 权重 | 96.50% → 96.49%，稀疏度 0.30 | 近乎无损 |
...
2. **压缩的价值（工程）**：蒸馏学生体积 **1/19**、延迟 **1/15**，测试集 **96.50% vs 教师 96.50%——
   打平**（差异在 ±0.1pp 评估波动内）；软标签携带类目相似度信息，相当于自带正则化。

**上线成果（蒸馏 BiLSTM）**

1. 测试集准确率 **96.50%**（与教师打平），全场最佳且体积极小；歧义/违禁品风险由置信度阈值 + 人工复核兜底
...
| 方案 | 准确率 | 单条延迟 | 单条成本 |
|---|---|---|---|
| DeepSeek（few-shot，零训练） | **80.39%** | ~2080 ms | 按 token 计费 |
| BERT 微调（教师） | 96.50% | 19.5 ms | 一次性训练 |
| **蒸馏 BiLSTM（上线）** | **96.50%** | **1.3 ms** | 零边际成本 |

**结论：在错字/混填/结构盲区的业务难度下，通用 LLM 提示工程比领域微调低 16.1 个百分点，延迟高三个数量级**——
