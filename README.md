...
| 压缩② | 知识蒸馏 | 教师 BERT → 学生 BiLSTM(128/256/2层)，KL·T²(T=2.0) + CE，α=0.7 | **96.46%（反超教师）**，20.9MB（约 **1/19**），CPU 推理 **1.3ms**（约 **16×**） | **最终上线口径** |
...
2. **压缩的价值（工程）**：蒸馏学生不仅体积 **1/19**、延迟 **1/16**，测试集还**反超教师 0.09pp**——软标签携带类目相似度信息，相当于自带正则化（Hinton 蒸馏理论的经典现象）。

**上线成果（蒸馏 BiLSTM）**

1. 测试集准确率 **96.46%**，全场最佳且体积极小；歧义/违禁品风险由置信度阈值 + 人工复核兜底
