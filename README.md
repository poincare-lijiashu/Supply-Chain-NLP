...
**难度构成（对齐真实运单痛点，防止"模板数据虚高"）**

- **错别字 12%**：同音/形近替换，每条最多 2 处（手写运单常态）
- **双物品混填 15%**："奶粉和合同一起寄"——按寄件申报规则取**第一项**定类目，词序互换构成最小对比对，考察语义优先级理解
- **属性线索**："玻璃瓶装的红酒"→ 按业务规则申报为易碎品，需结合容器材质常识推理
- **分布漂移**：验证/测试集混入训练未见物品词，易混样本加倍
- 歧义描述（"一箱东西"类）控制在 3-4%：此类运单业务上走人工补全通道
...
|---|---|---|---|---|
| 基线 | TF-IDF + 随机森林 | jieba 取前30词，2万条子集 | **87.52%** | 难度基准下词面特征不足 |
| 快速模型 | fastText（字/词 default） | 层次softmax，wordNgrams=1 | **89.11% / 89.71%**，单条 ~0.01ms | 毫秒级，但默认配置难以应对混填/错字 |
| 快速模型调参 | fastText autotune 300s | 自动搜索 wordNgrams/epoch/lr | **96.46% / 96.40%** | 可追平，但需逐场景调参 |
| 主模型 | BERT 微调 | bert-base-chinese + 全连接层，AdamW lr=5e-5，batch=128，4 epochs | **96.37%**，Macro F1 0.9650 | 零调参泛化，397MB |
| 压缩① | 动态量化 int8 | quantize_dynamic({Linear}) | **96.40%**，390→146MB（**2.68×**） | 精度无损 |
| 压缩② | 知识蒸馏 | 教师 BERT → 学生 BiLSTM(128/256/2层)，KL·T²(T=2.0) + CE，α=0.7 | **96.46%（与教师持平略优）**，20.9MB（约 **1/19**），CPU 推理 **1.3ms**（约 **16×**） | **最终上线口径** |
| 可选 | L1 非结构化剪枝 30% | encoder.query 权重 | 96.37% → 96.38%，稀疏度 0.30 | 近乎无损 |

**两条价值曲线，读表前先看这里**

1. **训练的价值（能力）**：难度基准（错字 12% + 双物品混填 15% + 属性线索）下，词面模型（RF/fastText 默认配置）只有 87.5~89.7%，BERT 靠预训练语义直接 96%+——**约 7 个百分点的差距就是深度模型训练的价值**；fastText autotune 虽能追平，但需 300s 逐场景调参、换分布即失效，而 BERT 零调参泛化。
2. **压缩的价值（工程）**：蒸馏学生体积 **1/19**、延迟 **1/16**，测试集 **96.46% vs 教师 96.37%——打平并略优**（差异在 ±0.1pp 评估波动内）；软标签携带类目相似度信息，相当于自带正则化（Hinton 蒸馏理论的经典现象）。

**上线成果（蒸馏 BiLSTM）**

1. 测试集准确率 **96.46%**（与教师打平略优），全场最佳且体积极小；歧义/违禁品风险由置信度阈值 + 人工复核兜底
