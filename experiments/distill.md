...
- 教师：bert-base-chinese 微调（难度基准 v1.3 · 4 epochs，Test Acc 96.52%，Macro F1 0.9676）
...
```
蒸馏-soft（难度基准 v1.3 · 4 epochs）:
  dev_acc 按 epoch: 0.9503 → 0.9503 → 0.9620 → 0.9644（epoch4 最优保存）
  Test Acc=0.9649, Macro F1=0.9660 —— 与教师（96.52%）打平（差异在 ±0.1pp 评估波动内）
...
- **学生追平教师**：软标签携带类目相似度信息（如「食品生鲜」与「医药健康」的接近程度），
  ...
  宜解读为"打平"）
- **压缩收益**：体积 1/19（20.9MB vs 397MB），CPU 推理 1.3ms（约 13×），精度无损
